...
`./run.sh stack` 이 죽는다. **원인은 포트 2342 점유**(`OSError: [Errno 98]`, 스파이크에서
남은 `python3 -m http.server`). 로직이 아니라 *점유를 감지도 보고도 안 하는 것* 이 결함이고,
URL 을 먼저 다 찍은 뒤 죽어서 성공처럼 보인다. grok 이 `~/repos/gh/nixos-config/run.sh` 모양
(색 헬퍼 · 번호 메뉴 · `main "$@"`)으로 다시 세우는 중이다. **그동안 `run.sh` 를 건드리지 마라.**

## 다음 홉 — 시인성

GLG 가 처음부터 지목한 자리다: *"hextra 스타일로 webtui css 를 잘 맞춰서 내가 원하는
시인성으로 넣을 수 있어."* 길은 확인됐다(`:post-process`). 아직 안 한 것은 **무엇을 어떻게
보이게 할지** 다.

주의: WebTUI base 의 `word-break:break-all` 이 한국어 산문을 아무 데서나 자른다.
**영어로만 보면 안 나타난다.**
