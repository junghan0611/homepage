...
**`github.com/junghanacs` 계정은 sameAs에서 뺐다(2026-07-13).** 가든이
`junghanacs/notes.junghanacs.com` → `junghan0611/garden` 으로 이관되고 Netlify
소스도 relink되면서, 거기 남은 건 read-only MOVED 저장소 / archived / fork 뿐이다.
활성 저장소가 없는 프로필은 신원 신호가 아니라 죽은 링크에 가깝다 — GitHub 축은
`junghan0611` 하나로 일원화한다. **`Person.sameAs`는 정체성 프로필 URL만 담으므로**
`junghan0611/garden` 같은 repo 링크를 대신 넣지 않는다. 가든의 정체성 면은 이미
`notes.junghanacs.com`이 담당한다.

> **사실 정정(2026-07-18, `gh api users/junghanacs` 실측).** `junghanacs`는 GitHub
> **organization이 아니라 별개의 legacy user 계정**(`"type": "User"`, public repo 9)이다.
...
`llms.txt`에서 "소유권"이라는 말을 강한 법적 의미로 쓰지 않는다.
...
`Person.sameAs` 배열은 인물 프로필 URL만 담는다**(아래 신원 절). 위키독스 book은 인물
프로필이 아니라 저작물이므로 그 배열에 들어가지 않는다. `Person.owns`도 답이 아니다 —
range가 `OwnershipInfo|Product`라 `WebSite`/`Book`에 쓰면 불변식 #1(표준만) 위반이다.
발행 자산은 **노드로 등재하고 person을 `author`/`publisher`로 가리키게** 한다.

**가든 `@id` 충돌 없음.** 가든이 emit하는 `notes.junghanacs.com/#website`와 같은 `@id`를
쓰되 `name`/`publisher`를 동일 의미로 맞춰 병합해도 무모순이다. 다만 `publisher`는
homepage `#person`(`junghanacs.com/#person`)을 가리키므로 병합 시 가든 `#person`과 둘이
된다 — 두 Person이 서로 `sameAs`로 이어져 있어 모순이 아니라 크로스도메인 신호다.

사람이 읽는 정본은 `static/llms.txt`. JSON-LD와 llms.txt는 **같은 사실을 두 채널로**
말한다. 한쪽만 고치지 말 것.
...
8. **자산 등록부는 홈에만**, `sameAs`가 아니라 노드+`author`/`publisher`로 표현. 미러는
   `isBasedOn`으로 파생임을 명시하고, **어떤 미러도 정본으로 승격하지 않는다.**
9. **소유권 사실은 JSON-LD와 `static/llms.txt` 양쪽에 같은 값으로** 존재한다. 한 채널만
   갱신하면 두 기계 독자가 서로 다른 소유권을 읽는다.
