...
- [x] **3. 불변 Eval engine release + claim-v1 실제 map 영수증**
- [ ] **4. branch 검토·merge·push** ← CURRENT: GLG 승인 대기
- [ ] **5. production 관측 뒤 대문 공사중 표식 철거** ← PAUSED: 배포 전

현재 좌표: 읽기판·엔진·candidate 검증 완료 → merge/push 승인 대기 → production gate 뒤 표식 철거

# NOW — SICM reading edition 출하

- Current: `sicm-reading-edition`은 `main`보다 15 commits 앞서며 `./run.sh v` 통과.
...
- 2026-09-12: Hugo build-time KaTeX→MathML rendered 512 Chapter 1 formulas per language with no
  ...
  `0.00016772069029036274 < 0.00017` and produced an 80-point SVG.
- 2026-09-12: Homepage-owned `/eval/engine/releases/2026.9.12/` freezes `cell-v1` and publishes
  `claim-v1` with scalar-exact, structured-field, and explicit-fragment modes. A real Scittle
  keyword map caught and retired one pre-release lookup assumption; final EN/KO candidate browser
  surfaces PASS. Both browser receipts remain local candidates, not production.
